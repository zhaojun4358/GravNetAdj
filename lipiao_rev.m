...
%      if app.InstrumentHightCheckBox.Value
      ...
      k = -(gRC3-gRC1)/(deg(i+2)- deg(i));           
      gRCend1 = (gRC1+ k*(deg(i)- deg(i)))*10^3;     
      gRCend2 = (gRC2 + k*(deg(i+1) - deg(i)))*10^3; 
      gRCend3 = (gRC3 + k*(deg(i+2) - deg(i)))*10^3; 
      %********************%

      %duancha = gezhi{1,1}(1)*((gRCend1 - gRCend2) + (gRCend3 -gRCend2))/2;
      %********************%
      duancha = ((gRCend2 - gRCend1) + (gRCend2 -gRCend3))/2; 
    ...
    elseif sst(st_num) == 4
           ...
           if app.OceanTideCheckBox.Value
            ...
            k = -(gRC4-gRC1-(gRC3-gRC2))/(deg(i+3)- deg(i)-(deg(i+2)- deg(i+1))); 
            gRCend1 = (gRC1+ k*(deg(i)- deg(i)))*10^3;    
            gRCend2 = (gRC2 + k*(deg(i+1) - deg(i)))*10^3; 
            gRCend3 = (gRC3 + k*(deg(i+2) - deg(i)))*10^3; 
            gRCend4 = (gRC4 + k*(deg(i+3) - deg(i)))*10^3; 

            %      duancha = gezhi{1,1}(1)*((gRCend1 - gRCend2) + (gRCend3 -gRCend2))/2;
            %********************%
            duancha = ((gRCend2 - gRCend1) + (gRCend3 -gRCend4))/2; 
     ...
     elseif sst(st_num) == 5
            ...
            if app_SolidTide
             ...
             if app_AtmosphericPressure
              ...
              if app.OceanTideCheckBox.Value
               ...
               %*****************************************************************
               %A-B¶Î
               k = -(gRC5-gRC1 )/(deg(i+4)- deg(i));           
               gRCend1 = (gRC1 + k*(deg(i)- deg(i)))*10^3;     
               gRCend2 = (gRC2 + k*(deg(i+1) - deg(i)))*10^3;  
               duancha1 = gRCend2 - gRCend1;                   
               %*****************************************************************
               %B-C¶Î
               gRCend3 = (gRC2 + k*(deg(i+1)- deg(i)))*10^3;   
               gRCend4 = (gRC3 + k*(deg(i+2) - deg(i)))*10^3;  
               duancha2 = gRCend4 - gRCend3;                   
            ...
            if mod(kk,2) == 1
               k = -(gRC(i) -  gRC(i+kk-1))/(deg(i)- deg(i+kk-1));  
               num_ducha = (kk-1)/2;
               for j = 1:num_ducha 
                   gRCend1 = (gRC((i-1)+j) + k*(deg((i-1)+j) - deg((i-1)+1)))*10^3;     
                   gRCend2 = (gRC((i-1)+j+1) + k*(deg((i-1)+j+1) - deg((i-1)+1)))*10^3;  
                ...
                k = -(gRC(i+kk-1) - gRC(i)-(gRC(i+j) - gRC(i+j-1)))/(deg(i+kk-1)-deg(i)- (deg(i+j) - deg(i+j-1)));  
                num_ducha = (kk-1)/2;
                for j = 1:num_ducha 
                   gRCend1 = (gRC((i-1)+j) + k*(deg((i-1)+j) - deg((i-1)+1)))*10^3;      
                   gRCend2 = (gRC((i-1)+j+1) + k*(deg((i-1)+j+1) - deg((i-1)+1)))*10^3;  
