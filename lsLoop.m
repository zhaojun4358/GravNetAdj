...
f = waitbar(0,'closed loop computation');
...
for i = 1:m_Lnumber 
        ...
        if neighbor(k1) > 0
           ...
           while 1
             ...
             W  = AAA{i,4} + diff(k1);
             SS = S(k1) + 1/PP(i);
             fprintf(fid,'The difference of closed loop%3d:',LoopClosureNum);
             fprintf(fid,'W =%9.4f   number of line = %2d  tolerance = %8.4f (2*sqrt(n)*m0)', -W, NumInOneLoop,2*sqrt(NumInOneLoop*1)*m_Sigma);
             if  abs(W) > 2*sqrt(NumInOneLoop*1)*m_Sigma
                 fprintf(fid,'%s   ','Superior tolerance');
             else
                 fprintf(fid,'%s','    ');
             end
        ...
        if i == m_Lnumber
           waitbar(i/m_Lnumber,f,{'The check for Loop closure error is complete' 'Operation Successfully'});
        else
